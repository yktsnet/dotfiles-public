# SOPS・秘密情報・デバイス追加 運用マニュアル（macbook / macOS版）

> **ビルドマシン: macbook（Apple Silicon, nix-darwin）**
...
```bash
# 1. 生ファイルを一時領域で作成・編集
# macOSには /dev/shm がないため tmpfs RAMディスクを作成する
...
> **簡易代替**: 作業が短時間で完了するなら `mktemp -d` の一時ディレクトリでも可。
> ただし macOS のデフォルト `/tmp` はディスクバック済みなので厳密には非RAMディスク。
...
> **注意（JSON/YAMLファイル全体を配置したい場合）:**
> credentials.json のように、JSONやYAMLの特定キーではなく「ファイル全体」を丸ごと復号して配置したい場合は、自動判定で `json`（キー抽出）として解釈されないようにする必要があります。
> この場合は、カテゴリを `devices/secrets.nix` の `legacyBinaryCategories` に追加した上で、明示的にバイナリ形式（`sops -e --input-type binary --output-type binary -i <ファイル>`）で暗号化して配置してください。
...
`.sops.yaml` を更新しただけでは既存ファイルに新デバイスの鍵が含まれない。
...
### `the key '<category>' cannot be found` エラー

原因:
JSONファイル全体を丸ごと復号する目的で、`secrets/<カテゴリ>/filename.json.age` のように自動スキャンで登録された場合に発生します。
`sops-nix` は `format = "json"` のシークレット名にスラッシュ（`/`）が含まれていると、それをJSONのキーパスと自動解釈して部分抽出を試みてしまい、該当するキーが見つからずにエラーになります。

対処:
対象のカテゴリを `devices/secrets.nix` の `legacyBinaryCategories` に追加し、ファイル全体がバイナリ（`format = "binary"`）として自動判定・復号されるようにします。
また、暗号化の際も `sops -e --input-type binary --output-type binary -i <ファイル名>` で明示的にバイナリ形式で暗号化してください。
