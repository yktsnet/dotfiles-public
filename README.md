...
```mermaid
graph TD
    classDef user fill:#82aaff,stroke:#333,stroke-width:1px,color:#1b1e28;
    classDef ai fill:#c792ea,stroke:#333,stroke-width:1px,color:#1b1e28;
    classDef doc fill:#fffac2,stroke:#333,stroke-width:1px,color:#1b1e28;

    WebChat[WebChat / 設計・対話型AI]:::ai -->|仕様策定 / チケット起票| Issue(Markdown Issue / 要件定義):::doc
    Issue -->|issue / jules コマンドで起動| Agent[AI Agent / 実装・自律型AI]:::ai
    Agent -->|コード自動編集 / テスト実行| PR(Pull Request):::doc
    PR -->|動作確認 / レビュー| User[User / 検証・人間]:::user
    User -->|issue-finish / jules-finish| Merge((PR Merge & Ticket Close)):::user
```
...
本リポジトリでは Nix Flakes および Home Manager をベースのインフラとして採用。MacBook（macOS）と Linux デスクトップにわたり、エージェントが利用するツールチェーン（Neovim, Yazi, Git, LSP等）や実行バイナリ、環境変数をコードとして完全に同一化。これにより、環境 of 差異によるエージェントの「コマンド未検出」「実行時エラー」を防止し、異なるOS間でのシームレスなAI協調開発の基盤を担保。

```mermaid
graph LR
    classDef flake fill:#addb67,stroke:#333,color:#1b1e28;
    classDef host fill:#89ddff,stroke:#333,color:#1b1e28;
    classDef tool fill:#fae4fc,stroke:#333,color:#1b1e28;

    Flake[flake.nix]:::flake --> macOS[macbook / macOS darwin]:::host
    Flake --> Laptop[linux-laptop / NixOS]:::host
    Flake --> Desktop[linux-desktop / NixOS]:::host
    Flake --> Servers[Servers / NixOS]:::host

    macOS --> HM[Home Manager]
    Laptop --> HM
    Desktop --> HM
    Servers --> HM

    HM --> Zsh[Zsh / aiagent.sh]:::tool
    HM --> Nvim[Neovim / lazy.nvim]:::tool
    HM --> Yazi[Yazi / Rust Filer]:::tool
    HM --> Tmux[Tmux / OSC 52]:::tool
```
...
* **Neovim (IDE & Editor)**: `lazy.nvim` によるポータブルで高度にモジュール化された開発環境。Ranger譲りのキーマップ、LSP、自動フォーマッタ（conform.nvim）、および Yazi とのシームレスな統合を提供。
* **Yazi (Terminal File Manager)**: Rust製の超高速ファイラー。fzf/ripgrep連携による高速検索や、終了時にシェルのカレントディレクトリを同期するラッパー関数を搭載。
* **Tmux (Terminal Multiplexer)**: プレフィックスキー不要のペイン切り替え、OSC 52による透過的なクリップボード同期、True Color対応など、リモート・ローカルの差異を埋める設定。
