...
Each repository's `docs/guarantees.md` remains the source of truth; this placement does not change. But running several repositories in parallel leaves no place to see "what does the fleet as a whole promise right now." A person approving guarantees who can't see the whole picture defeats the ledger's purpose, so on every rebuild a scan walks the search targets and gathers the ledgers into symlinks under one directory. Nothing is copied — the source of truth stays in each repository, and only references are gathered. When the same repository name appears under more than one search target, priority order resolves the conflict and lower-priority hits get a prefix from their search directory (see `apps/lpt/README.md` for the procedure).
