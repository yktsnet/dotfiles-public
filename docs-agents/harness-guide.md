...
**索引を挟んで差分だけ読む。** `.claude/RULES.md` に規則の中身は書かず、ファイルへのポインタ・一言要約・相互参照・前回棚卸し時点の内容ハッシュと日付だけを置く。2回目以降はそこを起点に、ハッシュが変わったファイルだけを深読みする。索引が無い設計では実行のたびに全対象を読むことになり、定期実行のコストが対象数に比例して積み上がる。記録するハッシュは「その内容を精読した」という宣言であり、精読していない行は触っても更新しない。裁可済みの意図的差分は別節に1行で残し、次回以降は指摘として再提起しない。
