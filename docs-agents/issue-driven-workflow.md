...
2. issues/ の変更を `main` へコミット（worktree ブランチに issue を含め、PR の diff に issues/ が混ざらないようにするため）。push は `issue-finish` が PR 作成前に行う。
3. Agentごとの分岐：
...
2. `main` と選択ブランチを push し、コミットメッセージ本文を PR 本文として `gh pr create` → `gh pr merge --merge`。
3. `git pull --prune` を実行（main のチェックアウトは常に main のまま）。
4. マージ済み `claude/*` の worktree・ローカル・リモートブランチを一括削除。
